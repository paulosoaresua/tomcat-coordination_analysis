...
VOCALIC_FEATURES = ["pitch", "intensity", "jitter", "shimmer"]
...
class VocalicSeries:
    ...
    def normalize_per_subject(self):
        ...
        for subject in all_subjects:
            ...
            self.observation[:, self.subjects_in_time == subject] = (
                obs_per_subject - mean
            ) / std
    ...
    def from_data_frame(cls, evidence_df: pd.DataFrame, vocalic_features: List[str]):
        ...
        for vocalic_feature in vocalic_features:
            obs_vocalic.append(
                np.array(literal_eval(evidence_df[f"{vocalic_feature}"].values[0]))
            )
        ...
        return cls(
            ...
            num_time_steps_in_coordination_scale=evidence_df[
                "num_time_steps_in_coordination_scale"
            ].values[0],
            subjects_in_time=np.array(
                literal_eval(evidence_df["vocalic_subjects"].values[0]), dtype=int
            ),
            observation=obs_vocalic,
            previous_time_same_subject=np.array(
                literal_eval(
                    evidence_df["vocalic_previous_time_same_subject"].values[0]
                ),
                dtype=int,
            ),
            previous_time_diff_subject=np.array(
                literal_eval(
                    evidence_df["vocalic_previous_time_diff_subject"].values[0]
                ),
                dtype=int,
            ),
            time_steps_in_coordination_scale=np.array(
                literal_eval(
                    evidence_df["vocalic_time_steps_in_coordination_scale"].values[0]
                ),
                dtype=int,
            ),
        )
...
class VocalicPosteriorSamples:
    ...
    def from_inference_data(cls, idata: Any) -> VocalicPosteriorSamples:
        coordination_posterior_samples = (
            CoordinationPosteriorSamples.from_inference_data(idata)
        )
...
class VocalicModel:
    ...
    def _define_pymc_model(self, evidence: VocalicSeries):
        coords = {
            ...
            "coordination_time": np.arange(
                evidence.num_time_steps_in_coordination_scale
            ),
    ...
    def prior_predictive(
        self, evidence: VocalicSeries, num_samples: int, seed: Optional[int] = None
    ):
        pymc_model = self._define_pymc_model(evidence)
        with pymc_model:
