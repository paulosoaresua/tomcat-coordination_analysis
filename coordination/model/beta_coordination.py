...
class BetaCoordinationInferenceFromVocalics(CoordinationModel, ParticleFilter):
    ...
    def fit(self, input_features: EvidenceDataset, num_particles: int = 0, num_iter: int = 0, discard_first: int = 0):
        # MCMC to train parameters? We start by choosing with cross validation instead.
        return self

    def predict(self, input_features: EvidenceDataset, num_particles: int = 0, *args, **kwargs) -> List[np.ndarray]:
