...
def logp(
    ...
    """
    This function computes the log-probability of a serial component. We use the following
    ...
    if self_dependent.eval():
        ...
        # We use this binary mask to zero out entries with no previous observations from the
        # subjects. We use this to determine the time steps that belong to the initial values of
        # the component. Each subject will have their initial value in a different time step hence
    ...
    total_logp = pm.logp(
        pm.Normal.dist(mu=blended_mean, sigma=sigma, shape=prev_other.shape), sample
    ).sum()
...
def random(
    ...
    """
    This function generates samples from of a serial component for prior predictive checks. We use
...
class SerialComponentParameters:
    def __init__(
        self, mean_mean_a0: np.ndarray, sd_mean_a0: np.ndarray, sd_sd_aa: np.ndarray
    ):
...
class SerialComponentSamples:
    def __init__(self):
        """
        If the density of observations is smaller than one, each time series will have a different
        number of time steps. So we store each one in a list instead of in the first dimension of
        a numpy array.
        """

        self.values: List[np.ndarray] = []

        # Number indicating which subject is associated to the component at a time (e.g. the
        # current speaker for a vocalics component).
        self.subjects: List[np.ndarray] = []

        # Time indices indicating the previous occurrence of the component produced by the same
        # subject and their most recent partner. For instance, the last time when the current
        # speaker talked and a different speaker.
...
class SerialComponent:
    """
    This class models a serial latent component which individual subject's dynamics influence that
    of the other subjects as controlled by coordination.
    """
    ...
    def __init__(
        ...
        self.parameters = SerialComponentParameters(
            mean_mean_a0=mean_mean_a0, sd_mean_a0=sd_mean_a0, sd_sd_aa=sd_sd_aa
        )

    @property
    def parameter_names(self) -> List[str]:
        names = [self.mean_a0_name, self.sd_aa_name]
    ...
    def draw_samples(
        ...
        fixed_subject_sequence: bool = False,
        ...
        if self.share_sd_aa_across_subjects:
            sd_aa = self.parameters.sd_aa.value[None, :].repeat(
                self.num_subjects, axis=0
            )
        else:
        ...
        for s in range(num_series):
            ...
            num_time_steps_in_cpn_scale = len(
                samples.time_steps_in_coordination_scale[s]
            )
            ...
            for t in range(num_time_steps_in_cpn_scale):
                ...
                for subject, time in prev_time_per_subject.items():
                    ...
                    samples.prev_time_diff_subject[s][t] = (
                        time
                        if samples.prev_time_diff_subject[s][t] == -1
                        else max(samples.prev_time_diff_subject[s][t], time)
                    )

                prev_time_per_subject[samples.subjects[s][t]] = t

            # Draw values from the system dynamics. The default model generates samples by
            # following a Gaussian random walk with blended values from different subjects
            # according to the coordination levels over time. Child classes can implement their
            # own dynamics, like spring-mass-damping systems for instance.
    ...
    def _draw_from_system_dynamics(
        ...
        for t in range(num_time_steps):
            ...
            if prev_time_same_subject[t] < 0:
                # Sample from prior. It is not only when t=0 because the first utterance of a
                # speaker can be later in the future. t=0 is the initial state of one of the
                # subjects only.
                ...
                if self.self_dependent:
                    ...
                    # When there's no self dependency, the component either depends on the
                    # previous value of another subject or a fixed value.
        ...
        if can_repeat_subject:
            ...
            if fixed_subject_sequence:
                ...
                transition_matrix[:-1, 1:] = (
                    np.eye(self.num_subjects) * time_scale_density
                )
    ...
    def _create_random_parameters(
        ...
        """
        This function creates the initial mean and standard deviation of the serial component
        distribution as random variables.
        """
    ...
    def update_pymc_model(
        ...
        logp_params = (
            ...
            *self._get_extra_logp_params(
                subjects, prev_time_same_subject, prev_time_diff_subject
            ),
        )
        logp_fn = self._get_logp_fn()
        random_fn = self._get_random_fn()
        serial_component = pm.DensityDist(
            self.uuid,
            *logp_params,
            logp=logp_fn,
            random=random_fn,
