...
class SigmoidGaussianCoordinationParameters:
    def __init__(self, sd_mean_uc0: float, sd_sd_uc: float):
        self.mean_uc0 = Parameter(
            NormalParameterPrior(mean=np.zeros(1), sd=np.array([sd_mean_uc0]))
        )
...
class SigmoidGaussianCoordination:
    """
    This class models a time series of continuous unbounded coordination and its bounded values
    (\tilde{C}).
    """
    ...
    def parameter_names(self) -> List[str]:
        return [self.mean_uc0_name, self.sd_uc_name]
    ...
    def draw_samples(
        self, num_series: int, num_time_steps: int, seed: Optional[int] = None
    ) -> CoordinationSamples:
        ...
        samples.unbounded_coordination = (
            norm(loc=0, scale=1).rvs(size=(num_series, num_time_steps))
    ...
    def update_pymc_model(
        ...
        coordination = pm.Deterministic(
            "coordination",
            pm.math.sigmoid(unbounded_coordination),
            dims=[time_dimension],
        )
