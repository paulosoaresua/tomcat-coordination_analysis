...
class DirichletGaussianCoordination3D(Coordination):
    """
    This class models a time series of continuous unbounded coordination (U) and its bounded
    version C = Dirichlet(exp(U)). Coordination here is (3, T) having the axes representing:
    individualism, coordination and common cause. Individualism controls how much each person
    is in sync with itself across time. Coordination controls how much each person is in sync with
    their partners across time. Finally, common cause controls how much each person is in sync
    with an external unknown source (e.g., the environment).
    """
    ...
    def __init__(
        ...
        super().__init__(
            ...
            parameters=DirichletGaussianCoordinationParameters3D(
    ...
    def draw_samples(
    ...
    ) -> DirichletGaussianCoordinationSamples3D:
        """
        ...
        coordination = np.empty_like(unbounded_coordination)
        exp_uc = np.exp(unbounded_coordination)
        for i in range(num_series):
            for j in range(exp_uc.shape[-1]):
                coordination[i, :, j] = dirichlet(exp_uc[i, :, j]).rvs()

        return DirichletGaussianCoordinationSamples3D(
    ...
    def create_random_variables(self):
        ...
        with self.pymc_model:
            ...
            if self.coordination_random_variable is None:
                ...
                tmp = pm.Dirichlet(
                    name="coordination_dirichlet_tmp",
                    a=ptt.exp(unbounded_coordination.T),
                    dims=[self.time_axis_name, self.facets_axis_name],
                )
                self.coordination_random_variable = pm.Deterministic(
                    name=self.uuid,
                    var=ptt.transpose(tmp),
                    dims=[self.facets_axis_name, self.time_axis_name],
                )
...
class DirichletGaussianCoordinationParameters3D(ModuleParameters):
    """
...
class DirichletGaussianCoordinationSamples3D(ModuleSamples):
