...
def logp(
    ...
    # We transform the sample using backward dynamics so that we learn to generate samples with
    # the underlying system dynamics. If we just compare a sample with the blended_mean, we are
    # assuming the samples follow a random gaussian walk. Since we know the system dynamics, we
    # can add that to the logp such that the samples are effectively coming from the component's
    # posterior.
...
class NonSerialMassSpringDamperComponent(NonSerialComponent):
    """
    This class models a non-serial latent mass-spring-damper component which individual spring's
    dynamics influence that of the other springs as controlled by coordination.
    """
    ...
    def __init__(
        ...
        # We assume the spring_constants are known but this can be changed to have them as latent
        # variables if needed.
