...
class Serial2DGaussianLatentComponent(SerialGaussianLatentComponent):
    ...
    def get_latent_variable_for_deterministic_motion(self,
        ...
        """
        values = []
        for t in self.time_steps_in_coordination_scale:
            prev_same_idx = self.prev_time_same_subject[t]
            if prev_same_idx < 0:
                mean_a0_time_idx = 0 if self.share_mean_a0_across_subjects else t
                prev_same = mean_a0[:, mean_a0_time_idx][:, None]
            else:
                prev_same = values[prev_same_idx]

            prev_other_idx = self.prev_time_diff_subject[t]
            if prev_other_idx < 0:
                values.append(prev_same)
            else:
                # Blend values from two subjects
                c = self.coordination_random_variable[t]
                prev_other = values[prev_other_idx]

                F = ptt.as_tensor(np.array([[1.0, 1.0], [0.0, 1.0]]))
                F = ptt.set_subtensor(F[1, 1], 1 - c)

                U = ptt.as_tensor(np.array([[0.0, 0.0], [0.0, 1.0]]))
                U = ptt.set_subtensor(U[1, 1], c)

                blended_value = ptt.dot(F, prev_same) + ptt.dot(U, prev_other)
