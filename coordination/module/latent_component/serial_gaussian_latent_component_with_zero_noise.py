...
class SerialGaussianLatentComponent(GaussianLatentComponent):
    ...
    def create_random_variables(self):
        ...
        if self.zero_noise:
            # No noisy version. We simplify estimation by applying deterministic motion updating.
            self.latent_component_random_variable = (
                self.get_latent_variable_for_deterministic_motion(mean_a0=mean_a0))
        else:
            ...
            # Mask with 1 for time steps where there is observation for a subject (subject
            # index >= 0)
    ...
    def get_latent_variable_for_deterministic_motion(self,
                                                     mean_a0: pm.Distribution) -> pm.Deterministic:
        """
        Creates and returns a random variable for the case where there's no noise in the random
        walk.

        @param mean_a0: (dimension x time) value at initial point per subject over time.
        @return: random variable.
        """
        coordination = self.coordination_random_variable[self.time_steps_in_coordination_scale]
        values = []
        for t in range(len(self.time_steps_in_coordination_scale)):
            prev_same_idx = self.prev_time_same_subject[t]
            if prev_same_idx < 0:
                mean_a0_time_idx = 0 if self.share_sd_a_across_subjects else t
                prev_same = mean_a0[:, mean_a0_time_idx][:, None]
            else:
                prev_same = values[prev_same_idx]

            prev_other_idx = self.prev_time_same_subject[t]
            if prev_other_idx < 0:
                values.append(prev_same)
            else:
                # Blend values from two subjects
                c = coordination[t]
                prev_other = values[prev_other_idx]
                blended_value = (1 - c) * prev_same + c * prev_other
                values.append(blended_value)

        return pm.Deterministic(name=self.uuid,
                                var=ptt.concatenate(values, axis=1),
                                dims=[self.dimension_axis_name, self.time_axis_name])
