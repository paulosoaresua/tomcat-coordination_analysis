...
class Scaler:
    ...
    def transform(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        if self._mean or self._std is None:
...
class SerialScaler:
    ...
    def fit(self, X: np.ndarray, subject_indices: np.ndarray, num_subjects: int):
        """
        Fits the data by computing and storing means and standard deviation.
        ...
        if self.normalization_method == NORMALIZATION_PER_SUBJECT_AND_FEATURE:
            self._mean = np.zeros_like(num_subjects, X.shape[0])
            self._std = np.zeros_like(num_subjects, X.shape[0])
    ...
    def transform(self, X: np.ndarray, subject_indices: np.ndarray) -> np.ndarray:
        """
        Transforms the data with the stored mean and standard deviation from a previous fit.
        ...
        if self.normalization_method == NORMALIZATION_PER_SUBJECT_AND_FEATURE:
            normalized_values = np.zeros_like(X)
            for subject in range(num_subjects):
                # Get values for a specific subject across time.
                idx = np.array(subject_indices[:X.shape[-1]]) == subject
                m = self._mean[subject][:, None]
                s = self._mean[subject][:, None]
                normalized_values[:, idx] = (X[:, idx] - m) / s
