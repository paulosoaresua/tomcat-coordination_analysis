...
from coordination.model.conversation_model import ConversationModel, ConversationSeries, \
    ConversationSamples
...
conversation_model = ConversationModel(num_subjects=NUM_SUBJECTS,
                                       ...
                                       sd_mean_a0=np.ones((NUM_SUBJECTS, 2)) * max(
                                           INITIAL_STATE_CONV[:, 0]),
                                       sd_sd_aa=np.ones(1),
                                       sd_sd_o=np.ones(1),
...
def plot_spring_data(ax: Any, data: np.ndarray, title: str = "", line_width: float = 1,
    ...
    for s in range(NUM_SPRINGS):
        ax.plot(tt, y_shift_fn(data[s, 0], s), label=f"Spring {s + 1}", color=COLORS_SPR[s],
...
def plot_conversation_data(ax: Any, data: ConversationSeries, title: str = "",
                           line_width: float = 1, y_shift_fn: Callable = lambda x, s: x):
    for s, name in enumerate(SUBJECT_NAMES):
        tt = np.array([t for t, subject in enumerate(data.subjects_in_time) if s == subject])
        ax.plot(tt, y_shift_fn(data.observation[0, tt], s), label=name, color=COLORS_CONV[s],
                linewidth=line_width, linestyle='solid')
    sns.despine(ax=ax)
    ax.set_title(title)
    ax.set_xlabel("Time Step")
...
def conversation_samples_to_evidence(
        samples: ConversationSamples) -> ConversationSamples:
...
if PLOT_SPRING:
    ...
    spring_model.draw_samples(num_series=1, num_time_steps=T_plot, coordination_samples=coordination,
                              seed=SEED).observation.values[0]
    ...
    noisy_spring_uncoordinated_data = \
    spring_model.draw_samples(num_series=1, num_time_steps=T, coordination_samples=coordination,
                              seed=SEED).observation.values[0]

    coordination = np.ones((1, T)) * 2 / 3
    noisy_spring_coordinated_data = \
    spring_model.draw_samples(num_series=1, num_time_steps=T, coordination_samples=coordination,
                              seed=SEED).observation.values[0]

    coordination = np.ones((1, T))
    noisy_spring_supercoordinated_data = \
    spring_model.draw_samples(num_series=1, num_time_steps=T, coordination_samples=coordination,
                              seed=SEED).observation.values[0]

    coordination = np.ones((1, T)) * 0.2
    noisy_spring_0_2_data = \
    spring_model.draw_samples(num_series=1, num_time_steps=T, coordination_samples=coordination,
                              seed=SEED).observation.values[0]

    coordination = np.ones((1, T)) * 0.8
    noisy_spring_0_8_data = \
    spring_model.draw_samples(num_series=1, num_time_steps=T, coordination_samples=coordination,
                              seed=SEED).observation.values[0]
    ...
    plot_spring_data(axs[0, 0], spring_uncoordinated_data, title=f"C = 0",
...
if PLOT_CONVERSATION:
    ...
    conversation_uncoordinated_data = conversation_samples_to_evidence(
        conversation_model.draw_samples(num_series=1, num_time_steps=T_plot,
                                        coordination_samples=coordination, seed=SEED,
    ...
    noisy_conversation_uncoordinated_data = conversation_samples_to_evidence(
        conversation_model.draw_samples(num_series=1, num_time_steps=T,
                                        coordination_samples=coordination, seed=SEED,
                                        fixed_subject_sequence=True))
    ...
    plot_conversation_data(axs[0, 0], conversation_uncoordinated_data, title=f"C = 0", line_width=1,
                           y_shift_fn=lambda x, s: x + s * 2.5)
    axs[0, 0].set_xlim(x_slice)
    axs[0, 0].plot([v_pos, v_pos], [-1, 7], linestyle="dotted", linewidth=0.7, color="black")
    plot_conversation_data(axs[0, 1], conversation_0_2_data, title=f"C = 0.2", line_width=1,
                           y_shift_fn=lambda x, s: x + s * 2.5)
    axs[0, 1].set_xlim(x_slice)
    axs[0, 1].plot([v_pos, v_pos], [-1, 7], linestyle="dotted", linewidth=0.7, color="black")
    plot_conversation_data(axs[0, 2], conversation_coordinated_data, title=f"C = 0.5", line_width=1,
                           y_shift_fn=lambda x, s: x + s * 2.5)
    axs[0, 2].set_xlim(x_slice)
    axs[0, 2].plot([v_pos, v_pos], [-1, 7], linestyle="dotted", linewidth=0.7, color="black")
    plot_conversation_data(axs[1, 0], conversation_0_8_data, title=f"C = 0.7", line_width=1,
                           y_shift_fn=lambda x, s: x + s * 2.5)
    axs[1, 0].set_xlim(x_slice)
    axs[1, 0].plot([v_pos, v_pos], [-1, 7], linestyle="dotted", linewidth=0.7, color="black")
    plot_conversation_data(axs[1, 1], conversation_supercoordinated_data, title=f"C = 1", line_width=1,
                           y_shift_fn=lambda x, s: x + s * 2.5)
    axs[1, 1].set_xlim(x_slice)
