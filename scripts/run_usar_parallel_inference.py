...
def parallel_inference(
    ...
    experiment_blocks = np.array_split(
        experiments, min(num_parallel_processes, len(experiments))
    )
    ...
    for i, experiments_per_process in enumerate(experiment_blocks):
        ...
        initial_coordination_arg = (
            f"--initial_coordination={initial_coordination} "
            if initial_coordination
            else ""
        )
        sd_uc_arg = f"--sd_uc={sd_uc} " if sd_uc else ""
        mean_a0_vocalic_arg = (
            f"--mean_a0_vocalic={mean_a0_vocalic} " if mean_a0_vocalic else ""
        )
        ...
        p_semantic_link_arg = (
            f"--p_semantic_link={p_semantic_link} " if p_semantic_link else ""
        )
        call_python_script_command = (
            f'python3 "{project_dir}/scripts/run_usar_sequential_inference.py" '
